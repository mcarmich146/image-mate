...
def _normalize_band_uint8(arr: np.ndarray, nodata_value: float | None = None) -> np.ndarray:
    values = arr.astype(np.float32, copy=False)
    finite = np.isfinite(values)
    if nodata_value is not None and math.isfinite(float(nodata_value)):
        finite = finite & (values != float(nodata_value))
...
def _read_raster_rgb_uint8(raster_path: str) -> tuple[np.ndarray, int, int]:
    with rasterio.open(raster_path) as ds:
        src_width = int(ds.width or 0)
        src_height = int(ds.height or 0)
        band_count = int(ds.count or 0)
        if src_width <= 0 or src_height <= 0 or band_count <= 0:
        ...
        for idx in range(pick_count):
            ...
            norm_bands.append(_normalize_band_uint8(data[idx], nodata))

        if len(norm_bands) == 1:
            norm_bands = [norm_bands[0], norm_bands[0], norm_bands[0]]
        elif len(norm_bands) == 2:
            norm_bands = [norm_bands[0], norm_bands[1], norm_bands[1]]

        image_rgb = np.stack(norm_bands[:3], axis=-1)
        return image_rgb.astype(np.uint8, copy=False), src_width, src_height
...
def _sample_out_of_unit_ratio(col: np.ndarray) -> float:
    if col.size > 2048:
...
def _select_obb_layout(arr: np.ndarray) -> str | None:
    if arr.shape[1] < 7 or arr.shape[1] > 50:
        return None
    col4_ratio = _sample_out_of_unit_ratio(arr[:, 4])
    col_last_ratio = _sample_out_of_unit_ratio(arr[:, -1])
...
def _parse_onnx_outputs(*, outputs: list[Any], input_width: int, input_height: int, conf: float) -> list[dict[str, Any]]:
    primary = None
    for row in outputs:
    ...
    obb_layout = _select_obb_layout(arr)
...
def _infer(*, layer_path: str, model_path: str, conf: float, iou: float, max_det: int) -> list[dict[str, Any]]:
    providers = []
    ...
    image_rgb, source_w, source_h = _read_raster_rgb_uint8(str(layer_path))
    ...
    sx = float(source_w) / float(target_w)
    sy = float(source_h) / float(target_h)
    for row in candidates:
        x1, y1, x2, y2 = row["bbox_px"]
        row["bbox_px"] = [
            max(0.0, min(float(source_w), float(x1) * sx)),
            max(0.0, min(float(source_h), float(y1) * sy)),
            max(0.0, min(float(source_w), float(x2) * sx)),
            max(0.0, min(float(source_h), float(y2) * sy)),
        ]
        obb = row.get("obb_px")
        if isinstance(obb, list) and len(obb) >= 4:
            ...
            for pair in obb[:4]:
                if not isinstance(pair, (list, tuple)) or len(pair) < 2:
                    continue
                try:
                    ox = max(0.0, min(float(source_w), float(pair[0]) * sx))
                    oy = max(0.0, min(float(source_h), float(pair[1]) * sy))
                    scaled.append([ox, oy])
                except Exception:
                    continue
    ...
    filtered = _nms(candidates, iou_threshold=float(iou))
    filtered.sort(key=lambda r: float(r.get("confidence", 0.0)), reverse=True)
    filtered = filtered[: int(max_det)]
        ...
        detections.append(
            {
                ...
                "class_name": f"class_{int(row.get('class_id', 0))}",
