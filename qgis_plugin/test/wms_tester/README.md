...
This test framework is under `qgis_plugin/test/wms_tester/`

## Usage

### Run all test cases:
```bash
cd qgis_plugin/test/wms_tester
python wms_diagnostic.py
```

### Run deterministic outcome gap/seam check (local proxy):
```bash
python outcome_gap_check.py
```

This check is tuned for the known problematic outcome and fails with exit code `1` if it detects
partial transparent seam rows/columns in the tested tile window.
